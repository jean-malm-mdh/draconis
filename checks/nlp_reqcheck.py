...
def remove_nonessential_words(words, words_to_remove):
    return [w for w in words if w.casefold() not in words_to_remove]
...
def classify_as_metric_check(tagged_words: List[Tuple[str, str]], numbers):
    ...
    enumeration, properties = iter_partition(
        lambda e: e.endswith("-"),
        [n for n, t in tagged_words if t.startswith("NN") or t.startswith("JJ")],
    )
    connector_word_index = get_index_by_property(
        lambda e: e == "and" or e == "or", lemmatized_words
    )
    if connector_word_index is not None and res["target_function"] != "interval":
        ...
        if (
            ...
            root_part = last_enum_word.replace(
...
def classify(req_text: str):
    ...
    words = remove_nonessential_words(
        token_words, set(filtered_stopwords).difference(stopwords_to_include)
    )
    lemmatized_words = [lemmatizer.lemmatize(word) for word in words]
    tagged_words = nltk.pos_tag(lemmatized_words)
    numbers = [
        n for n, t in tagged_words if t == "CD" or t == "NNP"
    ]  # CD = numeral/cardinal
    numbers.extend(
        [
            n
...
def complies(data, req):
    ...
    if classified_req["target_function"] == "interval":
        ...
        res = eval(
            f"{get_property_value(classified_req)} {classified_req['target_function']} {classified_req['val']}"
        )
        return res
